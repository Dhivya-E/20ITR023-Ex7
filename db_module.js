...
exports.insertData = function (name, email, eid, type, branch, response) {
    MongoClient.connect(url, function (err, db) {
        ...
        dbcon.collection("emp").insertOne(myobj, function (err, res) 
        {
            if (err)
            {
                console.log(err);
...
exports.retriveData = function (email, response) {
    MongoClient.connect(url, function (err, db) {
        ...
        console.log(query);
        dbcon.collection("emp").find({}).toArray(function (err, result) {
            if (err) {
                console.log(err);
                msg = "Error!!!";
            } else {
...
exports.updateData = function (eid, type, branch, response) {
    MongoClient.connect(url, function (err, db) {
        ...
        dbcon.collection("emp").updateOne(myquery, val, function (err, res) {
            if (err) 
            {
                console.log(err);
                msg = 'Error!!';
    ...
    MongoClient.connect(url, function (err, db)
     {
        ...
        var dbcon = db.db('WTdemo');
        var myquery = {'eid': eid};
        dbcon.collection("emp").deleteOne(myquery,function (err, res)
         {
            if (err) 
            {
                console.log(err);
...
exports.branchdata = function (email, response) {
    MongoClient.connect(url, function (err, db) {
        if (err) 
            throw err;
        ...
        console.log(query);
        dbcon.collection("emp").find({branch:'sales'}).toArray(function (err, result) {
            if (err) {
                console.log(err);
                msg = "Error!!!";
            } else {
